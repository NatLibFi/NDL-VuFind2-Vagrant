...
If all else fails, set _LOCAL_CACHE_CLEAR_ to **true** in _ubuntu.conf_ to clear local cache files. Remember to set this back to **false** to avoid clearing the cache every consecutive time the ubuntu virtual machine is being built.
